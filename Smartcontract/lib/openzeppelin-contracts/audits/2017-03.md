...
This function can only be called by the "Owner". As a first angle of attack, it's worth asking what will happen if the DagVault owners reset the daily limit by approving a call to `resetSpentToday`.
